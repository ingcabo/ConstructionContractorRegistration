...
<div class="headerbox"> 
	  ...
	  <?php $atributos = array('name' => 'form_personas', 'method'=>'post')?>
...
<td width="159"><input type="checkbox" name="comisario"  <?php echo $dis_comisario; ?>  id ="comisario"   <?php  echo $chk_comisario;?> <?php //if($acc === "t" and  $rp === "t" and $rp === "t") {echo 'checked disabled="true"';} else {echo 'disabled="true"';}?>         onclick="habilitaDeshabilita(this.form)" value="<?php echo $comisario;?>"/>
        ...
        <tr>
          ...
          <td><input type="checkbox" name="acc"    <?php echo $dis; ?> onclick="habilitaselect(this.form)"  <?php if($acc === "t"){ echo $checked; }else{echo "";}    ?>    value="<?php echo $acc;?>"/>
            Accionista
           <input type="text" name="porc" id= "porc" value="<?php $porc; ?>" size="2"  maxlength="5" onKeyUp="revisaCadena(this)" /><label>%</label>		   </td>
          <td>&nbsp;</td>
        </tr>
        <tr>
		...
		<input type="button" name="accion" value="Siguiente" onclick="enviar(3,<?php echo $valorpk; ?>)" />
	  <?php // echo anchor('con_reg_5/index/'.$valorpk.'/0/new', '<img alt="Siguiente" src="'.base_url().'system/application/imagenes/boton_continuar.gif" border="0">' );  ?>
		<?php $modo = '/new'; ?>
		...
		<input type="submit" name="boton" value="Guardar" onclick=""/> <input type="button" name="fin" value="Finalizar" onclick="enviar(6)" /></div></td>
        </tr>             
      </table>
	  </form>
