...
class Con_acceso extends Controller
{
    ...
    function index($modo,$tabla,$valor_pk = '',$frm_padre=''){
		...
		if($this->form_validation->run() == FALSE){
            ...
            if($modo === MODO_INCLUIR){
              ...
              //print_r($valores_nuevos);
              $npk  = $this->mod_usuario->nuevo($valores_nuevos);
              $vnpk = $npk->row_array();
              //print_r($vnpk);
              //echo 'valor: '.$vnpk['max'];
              //return;
              $this->menu_opciones($vnpk['max'],$modo);
            }
            if($modo === MODO_GRABAR){
              array_push($valores_nuevos,$this->input->post('txt_email'));
              array_push($valores_nuevos,$this->mylib_base->human_to_pg($this->input->post('txt_fecha')));
    ...
    function entrar()
      {
        ...
        $data = array(
            'title' => 'Sistema de Registro de Contratistas',
            'titulo'  => 'Bienvenidos al Sistema Registro y Seguimiento de Contratistas'
            );
        ...
        if ($this->form_validation->run() == FALSE )
        ...
        else
         {
			...
			if($centinela->login($nick, $password, $recordar)){
                ...
                $this->vismenu = $this->load->view('vis_menu','',TRUE);
				$this->load->view('vis_inicio');
            }else{
                $error = array('claveerronea'=>'Nombre de usuario o contraseña invalido, verifique.');
     ...
     function cambio_password(){
        $this->form_validation->set_rules('txt_anterior', 'Contrseña', 'callback__check_pass');
        ...
        $this->form_validation->set_rules('txt_passconfirma', 'Nueva Contrseña', 'trim|required|min_length[4]|max_length[15]|alpha_dash|xss_clean|matches[txt_passnuevo]');
        $this->vismenu = $this->load->view('vis_menu','',true);
         if ($this->form_validation->run() == FALSE ){
           $this->load->view('vis_cambio_password');
         }else{
    ...
    function _check_email($correo)
    {
        ...
        //devuelve error
        if ($q->num_rows() >= 1){
            $this->form_validation->set_message('_check_user', 'Correo electrónico asignado a otro usuario, utilice uno alternativo');
            return FALSE;
        }else{
            return TRUE;
        }
...
Se recomienda Personalizar esta contraseña a través de la opción Cambio de Contraseña ubicada del Menú Sistema del REGCO a fin de que pueda recordarla.";
    ...
    function menu_opciones($usuario_pk,$modo_act){
        ...
        if($modo_act === MODO_INCLUIR){
            ...
            $consulta = 'select um.usm_titulos,uo.usr_usuarios_fk,uo.uso_ver,uo.uso_incluir,uo.uso_modificar,uo.uso_borrar,uo.uso_campos,usm_usuarios_fk, um.usm_nivel, um.usm_activo, um.usm_usuarios_menu_pk
                         from sistema.usuarios_opciones as uo,sistema.usuarios_menu as um
                         where  um.usm_usuarios_menu_pk = uo.usm_usuarios_fk and usr_usuarios_fk = '.$usuario_pk.'  order by uso_usuarios_opciones_pk' ;
        }
            ...
            foreach($this->menuopciones as $fila){
               ...
               if($fila['usm_activo'] == 't'){
                 ...
                 if(isset($_POST['chk_bor'][$fila['usm_usuarios_menu_pk']]))
                   ...
                   $parametros = $fila['usm_usuarios_menu_pk'].',false,false,false,false)';
               }
               $consulta .=$parametros;
