...
class Con_reg_6 extends Controller
{
 ...
 function grabar(){
   ...
   if ($data['query_contratos_de_cada_contratista']->num_rows() > 0 and $this->uri->segment(4) > 0 ){ //
                        ...
                        $data['tipoact']          =$_POST['tipoact'];}else{
                        $data['tipoact']          ='';}

                        if(isset($_POST['orga'])){
                        $data['orga']          =$_POST['orga'];}else{
                        $data['orga']          ='';}

                        if(isset($_POST['estatus'])){
                        $data['estatus']          =$_POST['estatus'];}else{
                        $data['estatus']          ='';}

                        if(isset($_POST['fecha'])){
                        $data['fecha']          =$_POST['fecha'];}else{
                        $data['fecha']          ='';}

                       if(isset($_POST['numero'])){
                        $data['numero']          =$_POST['numero'];}else{
                        $data['numero']          ='';}

                       if(isset($_POST['rendimiento'])){
                        $data['rendimiento']          =$_POST['rendimiento'];}else{
                        $data['rendimiento']          ='';}

                        if(isset($_POST['descrip'])){
                        $data['descrip']          =$_POST['descrip'];}else{
                        $data['descrip']          ='';}
