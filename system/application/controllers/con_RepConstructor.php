...
class Con_repconstructor extends controller
{
    ...
    function AjaxMunicipios($campo_seleccionado){
         ...
         $objResponse->Assign("Divdpdciu","innerHTML", $this->mod_reportes->ciudades(0));
         return $objResponse;
    } //Fin Tipo_condicion
    ...
    function gen_reporte(){
        ...
        if(isset($_POST['bto_procesar']) or isset($_POST['bto_imprimir'])){
            $vista = 'SELECT c.*,ce.ett_nombre, nf.nfs_nivel, ct.ctp_descripcion, cl.clg_capital_social_suscrito,cl.clg_capital_social_pagado, dc.dec_denominacion_comercial_pk,dc.dec_descripcion
                        ...
                        and con_contratistas_pk = cl.con_contratistas_fk and cl.dec_denominacion_comercial_fk = dc.dec_denominacion_comercial_pk and cl.clg_modificacion = false';

            $filtro = '';
            //Filtrado por numero de rif
            if(isset($_POST['txt_rif']) and $_POST['txt_rif'] <> ''){
                $filtro = $filtro.' AND c.con_numero_rif = \''.$this->input->post('inirif').trim($this->input->post('txt_rif')).'\'';}
           ...
           //Filtrado por contratos del año seleccionado y contratado por
            ...
            //$cent = new Centinela();
            if(isset($_POST['bto_imprimir'])){
              $q_vista['pdf'] = true;
              $html = $this->load->view('reportes/rep_constructor',$q_vista, true);
            ...
            show_error('No hay datos para mostrar');
        }

    }

    //Imprime ficha en pdf
    function rep_ImpFicha() {
      if(isset($_POST['con_id'])){
             ...
             $html = $this->load->view('reportes/rep_ficha',$data,true);
